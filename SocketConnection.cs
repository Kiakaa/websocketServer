...
namespace WebSocketServer
{
    public class SocketConnection
    {
        ...
        //WebSocketServer用
        public event NewConnectionEventHandler scNewConnection;
        public event DataReceivedEventHandler scDataReceived;
        public event DisconnectedEventHandler scDisconnected;
        ...
        private void Read(IAsyncResult status)
        {
            ...
            try
            {
                ...
                // Convert.ToChar(65533):1111101十进制125，
                    ...
                    messageReceived.Length == 0)
                {
                    ...
                    if (scDisconnected != null)
                        scDisconnected(this, EventArgs.Empty);
                }
                else
                {
                    if (scDataReceived != null)
                    {
                        logger.Log("接受到的信息 [\"" + messageReceived + "\"]");
                        scDataReceived(this, messageReceived, EventArgs.Empty);
                    }
        ...
        public void ManageHandshake(IAsyncResult status)
        {
            ...
            //现在使用的是比较新的Websocket协议：第9点 请求必须包含一个名字为|Sec-WebSocket-Version|的头字段。该头字段的值必须是13。
            if (rawClientHandshake.IndexOf(header) != -1)
            {
                ...
                foreach (string Line in rawClientHandshakeLines)
                {
                    ...
                    //第7点 请求必须包含一个名字为| Sec-WebSocket-Key |的头字段
                    if (Line.Contains("Sec-WebSocket-Key:"))
                    {
                        //4.2.2 第5点 如果服务器选择接受传入的连接，它必须以一个有效的HTTP响应应答。
        ...
        private void HandshakeFinished(IAsyncResult status)
        {
            ...
            ConnectionSocket.BeginReceive(receivedDataBuffer, 0, receivedDataBuffer.Length, 0, new AsyncCallback(Read), null);
            //if (scNewConnection != null) scNewConnection(name, EventArgs.Empty);
            scNewConnection?.Invoke(name, EventArgs.Empty);
        }
    }
}